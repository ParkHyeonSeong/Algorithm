...
임한수의 번호가 순서대로 주어진다. N은 100,000보다 작거나 같은 자연수이고,
...
def act(n,a,b):
	...
	if a > b:		# 계산하기 쉽게 자리 변경
		temp = a
		a = b
		b = temp

	if a == b+1 :	# 홀수일때 바로 옆인지 확인하고
			return 1

	while flag == 0:
		if ((high+low)//2)//2 >= a and ((high+low)//2)//2 >= b:	# 필요없는 루트를 제외하도록하자
			high = high // 2
		elif ((high+low)//2)//2 < a and ((high+low)//2)//2 < b: # 필요없는 루트를 제외하도록하자
			low = low // 2
		else:						# 필요없는 루트가 없다? 그러면 무조건 최대횟수




n,a,b = input().split()	# 참가자의 수, 김지민, 임한수
print(act(int(n),int(a),int(b)))